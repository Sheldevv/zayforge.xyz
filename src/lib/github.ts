...
function classifyAsset(name: string): { platform: GitHubAsset['platform']; type: GitHubAsset['type'] } {
  ...
  if (lower.includes('.exe') || lower.includes('.msi')) {
    platform = 'windows';
    type = lower.includes('setup') || lower.includes('install') ? 'installer' : 'portable';
...
async function fetchRelease(owner: string, repo: string): Promise<ReleaseInfo | null> {
  const url = `https://api.github.com/repos/${owner}/${repo}/releases/latest`;
  const headers: Record<string, string> = { 'User-Agent': 'ZayForge-Website/1.0' };
...
export async function getLauncherRelease(): Promise<ReleaseInfo | null> {
  return fetchRelease('Sheldevv', 'ZayForge-Launcher');
}
